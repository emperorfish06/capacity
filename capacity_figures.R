...
png("C:\\Clock\\fig4.png", units="in", width=8, height=5, res=300)
fig4=read.csv("C:\\Clock\\fig4.csv",sep=",")
dea.plot(peru$Total_EffEffort_GearSpecies,peru$Total_Catch_GearSpecies,RTS="vrs",ORIENTATION="in-out",xlab="Effort",ylab="Catch")
dea.plot(peru$Total_EffEffort_GearSpecies,peru$Total_Catch_GearSpecies,RTS="crs",ORIENTATION="in-out",add=T,lty="dotted",col="blue")

dev.off()
#figure 5
fig5        = read.csv("C:\\Clock\\1st_paper\\New folder\\fig5.csv")
png("C:\\Clock\\fig5.png", units="in", width=7, height=8, res=300)
#write.csv(fig5,"C:\\Clock\\1st_paper\\New folder\\fig5.csv")
ggplot(subset(fig5, !(variable %in% c("TEvrs","TEcrs"))), aes(year, value, group=variable, colour=variable))+geom_line(size=1.5)+theme_bw()+
